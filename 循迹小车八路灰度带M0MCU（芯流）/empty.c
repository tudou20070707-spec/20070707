...
#define BTN_DEBOUNCE_CYC    50U                 /* 50ms / 1ms = 50         */
#define BTN_TIMEOUT_CYC     2000U               /* 2000ms / 1ms = 2000     */
#define BTN_LONG_PRESS_CYC  1500U               /* 1.5s / 1ms = 1500       */
...
#define DIRECTION_DEADBAND   0.05f              /* 超过死区才更新回找方向       */
#define LOST_OUTER_DUTY      0.30f              /* 丢线时外轮占空比             */
#define LOST_MAX_TICKS       2000U              /* 丢线超时 2s (1ms×2000)      */

/*===========================================================================
 * 速度 PID 参数 (慢速层 — 编码器调速)
 *===========================================================================*/
#define BASE_DUTY_NOMINAL    0.20f              /* 巡航占空比初始值            */
#define BASE_DUTY_MIN        0.30f              /* PID 输出下限                */
#define BASE_DUTY_MAX        0.65f              /* PID 输出上限                */
#define SPEED_WINDOW_TICKS   50U                /* 调速周期 = 50×1ms = 50ms    */
#define SPEED_TARGET_PPS     80U                /* 目标: 每窗口 80 脉冲        */
...
int main(void)
{
    ...
    GrayscaleSensor gs         = {0};
    GrayscaleReadStatus gs_status      = GS_READ_INVALID_ARG;
    GrayscaleReadStatus last_gs_status = GS_READ_INVALID_ARG;
    float    line_error        = 0.0f;
    float    last_error        = 0.0f;
    uint32_t lost_timer        = 0;
    uint32_t gs_fail_total     = 0;
    uint32_t gs_fail_consecutive = 0;
    uint32_t gs_status_report_tick = 0;
    bool     have_valid_line   = false;
    bool     have_direction_history = false;
    ...
    /* 灰度传感器初始化 (芯流 T8, I2C 地址 0x40, 与 OLED 共用 I2C1) */
    Grayscale_Init();
    ...
    while (1)
    {
        ...
        case STATE_IDLE:
            ...
            if ((tick % 500U) == 0U) {   /* 每 500ms 打印一次 */
                GrayscaleSensor gs_test;
                GrayscaleReadStatus test_status = Grayscale_Read(&gs_test);
                if (test_status == GS_READ_OK) {
                    UART_Printf("[TEST] GS:%d%d%d%d%d%d%d%d | active:0x%02X | online:%d\r\n",
                                gs_test.x1, gs_test.x2, gs_test.x3, gs_test.x4,
                                gs_test.x5, gs_test.x6, gs_test.x7, gs_test.x8,
                                (unsigned)Grayscale_ActiveMask(&gs_test),
                                Grayscale_OnLineCount(&gs_test));
                } else {
                    UART_Printf("[TEST] GS_ERR:%d\r\n", (int)test_status);
                }
            }
        ...
        case STATE_COUNTING:
            ...
            if (idle_ticks >= BTN_TIMEOUT_CYC) {
                ...
                last_error     = 0.0f;
                lost_timer     = 0;
                gs_status      = GS_READ_INVALID_ARG;
                last_gs_status = GS_READ_INVALID_ARG;
                gs_fail_total  = 0;
                gs_fail_consecutive = 0;
                gs_status_report_tick = 0;
                have_valid_line = false;
                have_direction_history = false;
                /* 重置速度 PID 和测速基准 */
                PID_Reset(&speed_pid);
                g_base_duty       = BASE_DUTY_NOMINAL;
                last_speed_pulses = SpeedSensorA_GetTotalPulses();
        ...
        case STATE_RUNNING:
        {
            ...
            if (gs_status != last_gs_status &&
                (tick - gs_status_report_tick) >= 100U) {
                UART_Printf("GS status:%d -> %d\r\n",
                            (int)last_gs_status, (int)gs_status);
                last_gs_status = gs_status;
                gs_status_report_tick = tick;
            }
            ...
            if (gs_status != GS_READ_OK) {
                ...
                line_lost  = Grayscale_IsLineLost(&gs);
                bool all_black = (Grayscale_OnLineCount(&gs) >= 6);
                ...
                if (!line_lost && !all_black) {
                    ...
                    if (fabsf(line_error) > DIRECTION_DEADBAND) {
                        last_error = line_error;
                        have_direction_history = true;
                    }

                    /* 差速分级 (柔化版) */
                    float abs_err = fabsf(line_error);
                    float inner_ratio;
                    if      (abs_err < 0.08f) inner_ratio = 1.0f;
                    else if (abs_err < 0.25f) inner_ratio = 0.6f;
                    else if (abs_err < 0.55f) inner_ratio = 0.3f;
                    else                       inner_ratio = 0.1f;
                    ...
                    if (!have_valid_line || !have_direction_history) {
                        /* 尚无可信方向历史，禁止任意选择一侧旋转。 */
                        lost_timer = 0;
                    } else if (lost_timer >= LOST_MAX_TICKS) {
                        /* 回找超时后保持停车，等待赛道重新出现。 */
                        lost_timer = LOST_MAX_TICKS;
                    } else {
                        lost_timer++;
                        if (last_error > 0.0f) {
                ...
                /* ---- 固定基速 (暂时禁用速度PID, 防止基速过低) ---- */
                (void)speed_window_cnt;
                (void)last_speed_pulses;
                (void)speed_pid;
                /* g_base_duty 保持 BASE_DUTY_NOMINAL = 0.20 不变 */
            }
            ...
            if (lap_count == 0) {
                if ((tick % 1000U) == 0U) {
                    UART_Printf("GS:%d%d%d%d%d%d%d%d err=%+1.2f st=%d | "
                                "out:L=%3u R=%3u B=%3u | "
                                "fail:%lu/%lu dir:%d lost:%lu | "
                                "pulses:%lu INF | %s\r\n",
                                gs.x1, gs.x2, gs.x3, gs.x4,
                                gs.x5, gs.x6, gs.x7, gs.x8,
                                line_error, (int)gs_status,
                                g_pwm_a, g_pwm_b,
                                (uint8_t)(g_base_duty * 100.0f),
                                gs_fail_consecutive, gs_fail_total,
                                have_direction_history ? 1 : 0, lost_timer,
                                current_pulses,
                                (gs_status != GS_READ_OK) ? "GS_ERR" :
                                (!have_valid_line ? "WAIT_GS" :
                                ((follow_mode == MODE_NORMAL) ? "NORM" :
                                ((lost_timer >= LOST_MAX_TICKS) ? "LOST_STOP" : "LOST"))));
        ...
        case STATE_DONE:
            ...
            /* 200ms ON / 120ms OFF 闪烁 */
            if (flash_count < 6) {
                uint32_t phase = flash_timer % 320U;   /* 320ms 周期         */
                if (phase < 200U) {
                    OLED_DisplayOn();
                } else {
                    OLED_DisplayOff();
