 ...
 *
 *   上电: 电机停止, 数码管显示 "0"
 *   按按钮: 每按一次圈数+1 (1~9), 数码管实时显示
 *   2秒不按: 锁定圈数, 电机启动, 编码器计距
 *   走完: 电机停止, 数码管闪烁 3 次, 回到待机
 *
 *   距离: 1 圈 = 4 米
 *   编码器: 13 PPR, 1:20 减速比, 48mm 轮径
 *   数码管: 0.56" 共阳 (5611BS), PA1~PA7
 *   按钮: PA0, 内部上拉, 按下=GND, 50ms 消抖
 */
...
#define PULSES_PER_LAP      6896U               /* 4000/150.8 × 260 ≈ 6896 */
 ...
 * 速度 PID 参数 (慢速层 — 编码器调速)
 *===========================================================================*/
#define BASE_DUTY_NOMINAL    0.22f              /* 巡航占空比初始值            */
#define BASE_DUTY_MIN        0.10f              /* PID 输出下限                */
#define BASE_DUTY_MAX        0.50f              /* PID 输出上限                */
...
int main(void)
{
    ...
    /* 速度 PID 初始化: Kp=0.005, Ki=0.002, Kd=0, dt=50ms, 输出限幅 [0.10, 0.50] */
    ...
    while (1)
    {
        ...
        case STATE_RUNNING:
        {
            ...
            if (lap_count == 0) {
                if ((tick % 500U) == 0U) {
                    UART_Printf("GS:%d%d%d%d%d err=%+1.2f | "
                                "out:L=%3u R=%3u B=%3u | "
                                "pulses:%lu INF | %s\r\n",
                                gs.r2, gs.r1, gs.m, gs.l1, gs.l2,
                                line_error,
                                g_pwm_a, g_pwm_b,
